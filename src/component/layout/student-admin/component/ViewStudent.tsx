...
export const ViewStudent = () => {
    const [filteredStudent, setFilteredStudent] = useState([] as IStudents[]);

    const navigate = useNavigate();

    const getAllStudent = () => {
        StudentService.getAllStudents().then((response) => {
            const sortedStudents = response.data.sort((a, b) => Number(a.rollNo) - Number(b.rollNo));
            setFilteredStudent(sortedStudents);
        }).catch((error) => {
            console.log(error);
        });
    };
            ...
            <div className="container">
                <div className="row">
                    <div className="col-sm-6">
                        <div className="col">
                            <Link className="btn btn-success" to={'/student/add'}>
                            ...
                            <table className="table table-striped table-hover shadow-lg">
                                ...
                                {filteredStudent.map((student) => (
                                    <tr key={student.id}>
                                        ...
                                        <td>
                                            <Link to={`/student/edit/${student.id}`} className="btn btn-warning">
